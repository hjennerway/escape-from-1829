...
Batch generation currently happens once during page setup. A future build step could serialize these material batches and tree templates to binary assets to reduce startup work. Simply exporting the same unbatched geometry would not remove its per-frame draw submissions. Runtime changes to batched buildings would need to rebuild their affected batches and refresh cached transforms before invalidating shadows.
