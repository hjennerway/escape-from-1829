...
public class Pursuer : MonoBehaviour
{
    ...
    public void Configure(EscapeGame owner,int type)
    {
        ...
        Color color=kind==1?new Color(.08f,.13f,.22f):new Color(.49f,.82f,.74f);
        ...
        if(kind==2)
        {
            var signObject=new GameObject("Deva ghost sign"); signObject.transform.SetParent(transform,false); signObject.transform.localPosition=new Vector3(0,1.18f,.30f);
            var sign=signObject.AddComponent<TextMesh>(); sign.text="Deva ghost"; sign.fontSize=48; sign.characterSize=.045f; sign.anchor=TextAnchor.MiddleCenter; sign.alignment=TextAlignment.Center; sign.color=new Color(.67f,1f,.85f);
            var plaque=EscapeGame.Primitive("Sign plaque",PrimitiveType.Cube,signObject.transform.position+new Vector3(0,0,-.035f),new Vector3(1.02f,.23f,.04f),new Color(.08f,.28f,.23f)); plaque.transform.SetParent(transform,true); plaque.transform.SetSiblingIndex(0);
        }
    ...
    void Update()
    {
        ...
        if(sight||kind==2){destination=game.Player.position;memory=5;}
        ...
        if(rethink<=0)
        {
            ...
            {
                int[] xs=new[]{32,8,8,32};
                int[] zs=new[]{26,26,5,5};
        ...
        float speed=kind==1?(memory>0?3.85f:2.4f):2.2f;
