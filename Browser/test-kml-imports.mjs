...
for(const {name,xml} of placemarks('kml-trees/1829-13.kml').filter(p=>/^Oak(3[1-9]|4[0-4])$/.test(p.name))){
  const coordinates=xml.match(/<Point>[\s\S]*?<coordinates>([^<]+)/)[1].trim().split(',').map(Number);
  points.push({name,coordinates});
}
assert.deepEqual(KML_TREE_POINTS,points,'Preserve exact tree Point triples from each source export');
assert.equal(KML_TREES.length,72);assert.equal(KML_PINE_TREES.length,14);assert.equal(KML_OAK_TREES.length,48);assert.equal(KML_BEECH_TREES.length,2);
...
assert.equal(new Set(KML_TREES.map(t=>t.rotation)).size,72,'Each mapped tree has a distinct stable rotation');
...
console.log(`PASS: 72 exact KML trees in all periods, duplicate Oak8/Oak16/Oak21/Oak22 points, preserved front beeches, shared visibility and collisions; eight-vertex Countess roundabout (${area.toFixed(1)} square units), upward roads, walkable centre and continuous Modern-only approach.`);
