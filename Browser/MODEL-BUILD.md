...
Both compiled-scene browser scripts allow 120 seconds per navigation (including reloads and the walking page), screenshot and browser interaction, matching the existing scene-readiness timeout. Navigation and other operations have separate Playwright defaults, so both are configured explicitly. Full-detail startup and screenshot capture under software WebGL can exceed the default 30-second limit. The full-detail check also verifies compiled loading and records its timing in `precompiled-models.json`; rendering, image comparison and fallback assertions remain enabled.
