...
export function addOakmereWestElevation(THREE,{model,host,brick,roof,material,worldUV,hipRoof}){
 const depth=host.d;
 // Keep the photographed elevation's proportions in its own frame, then fit
 // its length to the west range of the existing open rear courtyard.
 host={...host,d:27*Math.hypot(-5299/6749,-9568/6749)};
 const group=new THREE.Group();group.name='Oakmere west lawn elevation';
 ...
 // the registered courtyard range and its inner face and end joins.
 const half=host.d/2,centreWidth=15.8,leftWidth=half-centreWidth/2,eaves=10.4;
 // This photo resolves the formerly generic courtyard face: its two storeys
 // stand higher than the neighbouring ward ranges. Add the upper masonry
 // above the saved body, keeping the courtyard and its inner face intact.
 mesh(worldUV(new THREE.BoxGeometry(host.d,eaves-host.h,host.w),1.7),brick,0,(eaves+host.h)/2,-host.w/2,'Oakmere west raised range upper masonry');
 const mainCap=hipRoof(0,-host.w/2+.3,host.d,host.w+.6,eaves,2.7);group.add(mainCap);mainCap.name='Oakmere west raised range slate roof';
 wall(-(half+centreWidth/2)/2,.35,leftWidth,eaves,.6,'Oakmere west four-bay left face');
 wall(0,.55,centreWidth,eaves,.8,'Oakmere west five-bay gabled face');
 wall((half+centreWidth/2)/2,.35,leftWidth,eaves,.6,'Oakmere west four-bay right face');
 const columns=[...[-19.8,-16.25,-12.7,-9.15].map(x=>({x,z:.67})),...[-6.4,-3.2,0,3.2,6.4].map(x=>({x,z:.97})),...[9.3,12.65,16,19.35].map(x=>({x,z:.67}))];
 for(const p of columns){sash(p.x,2.65,p.z,{h:3.4,w:1.8});sash(p.x,7.85,p.z,{blind:p.blind,h:3.1,w:1.8});}
 for(const [x,w,z] of [[-(half+centreWidth/2)/2,leftWidth,.72],[0,centreWidth,1.02],[(half+centreWidth/2)/2,leftWidth,.72]]){
  for(const [y,h] of [[.24,.4],[5.2,.4],[eaves-.24,.35]])box(trim,x,y,z,w,h,.15);
  box(pale,x,5.45,z+.02,w,.075,.08);
 }
 // Raised cross gable, terracotta raking bands and a recessed round vent.
 const base=eaves,rise=3.1,front=.99,back=-host.w/2;
 ...
 mesh(worldUV(new THREE.ExtrudeGeometry(triangle,{depth:1.0,bevelEnabled:false}),1.7),brick,0,base,front-1,'Oakmere west central brick pediment');
 const v=[[-centreWidth/2-.2,base,front+.17],[0,base+rise,front+.17],[centreWidth/2+.2,base,front+.17],[-centreWidth/2-.2,base,back],[0,base+2.7,back],[centreWidth/2+.2,base,back]],positions=[],uv=[];
 ...
 mesh(g,roof,0,0,0,'Oakmere west cross-gable slate roof');
 for(const side of [-1,1])for(const inset of [0,.27])beam([side*(centreWidth/2+.03),base-inset,front+.18],[0,base+rise-inset,front+.18],.14,trim,'Oakmere west pediment raking band');
 for(const y of [base+.36,base+1.05]){const w=centreWidth*(1-(y-base)/rise)-.18;box(trim,0,y,front+.07,w,.14,.14);}
 const ventY=base+1.75;
 mesh(new THREE.CircleGeometry(.48,32),iron,0,ventY,front+.045,'Oakmere west recessed circular vent');
 mesh(new THREE.TorusGeometry(.52,.1,8,32),pale,0,ventY,front+.12,'Oakmere west circular vent stone ring');
 mesh(new THREE.TorusGeometry(.68,.06,8,32),trim,0,ventY,front+.1,'Oakmere west circular vent brick ring');
