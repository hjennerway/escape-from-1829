...
export const ANNEXE_LOOP_ROAD=Object.freeze({
 ...
 start:Object.freeze([333.63,-84.91]),
 end:SHARED_HISTORIC_LANES.find(r=>r.name==='Parsons Lane (North)').points.at(-1)
});
...
const [baseA,baseB]=[start,[355,-107.85]].map(previousTrianglePoint).map(annexeSiteLocal);
...
export function alignAnnexeFrontage(point){
 const [x,z]=annexeSiteLocal(point),dx=x-anchorX,dz=z-anchorZ;
 const p=annexeSitePoint(anchorX+(dx+baseSlope*dz)/baseLength,0,
  ANNEXE_FRONT_ALIGNMENT.avenueZ+(-baseSlope*dx+dz)/baseLength);
 return [p[0],p[2]];
}
export function annexeTrianglePoint(point){return alignAnnexeFrontage(previousTrianglePoint(point));}
// The latest blue triangle sits on the outer-road side of the frontage.
// Extend the unchanged parallel avenue to its actual outer-road intersection.
const near=annexeTrianglePoint([355,-107.85]);
const previousCorner=annexeTrianglePoint(start);
const avenueSlope=(near[1]-previousCorner[1])/(near[0]-previousCorner[0]);
const outerSlope=(end[1]-start[1])/(end[0]-start[0]);
const joinX=(start[1]-near[1]+avenueSlope*near[0]-outerSlope*start[0])/(avenueSlope-outerSlope);
export const ANNEXE_FRONT_OUTER_JOIN=Object.freeze([joinX,annexeOuterRoadZ(joinX)]);
export const ANNEXE_TRIANGLE_CORNERS=Object.freeze([start,ANNEXE_FRONT_OUTER_JOIN,near]);
export const ANNEXE_TRIANGLE_FORK=Object.freeze([near,start]);
