...
export function createIrbyAshley(THREE,{brick,roof,worldUV,material}){
  ...
  const local=IRBY_ASHLEY_FOOTPRINT.map(([x,z])=>[x-cx,z-cz]);
  ...
  const mainFootprint=local.filter((_,i)=>i!==13&&i!==14);
  ...
  mass(projection,4,0,brick,'Low east service room').userData.historicOutlinePadding=.7;
  ...
  for(const spec of IRBY_ASHLEY_ROOFS){
  ...
  // Join the wider garden gable to the narrower north wing at the stepped wall.
  surface([[247.52-cx,eave,-124.35-cz],[250.65-cx,eave+1.18,-124.35-cz],[250.65-cx,eave+1.18,-122.5-cz],[247.52-cx,eave,-122.5-cz]],[[0,1,2],[0,2,3]],roof,'West wing roof junction',true);
  // The garden wall in img3 has two canted, full-height brick window bays.
  for(const bx of [241.8,225.1]){
    ...
    for(let i=0;i<5;i++){
      ...
      for(const y of (bx===241.8?[5.9]:[2.05,5.9]))sash((a[0]+b[0])/2+Math.sin(r)*.025,y,(a[1]+b[1])/2+Math.cos(r)*.025,r,i===2?1.35:.86,2.7,'Canted bay sash');
    }
  ...
  for(let i=0;i<mainFootprint.length;i++){
    ...
    const nx=dz/length,nz=-dx/length,r=Math.atan2(nx,nz);trim(a,b,{outward:-1});
    const wx=(a[0]+b[0])/2+cx,wz=(a[1]+b[1])/2+cz;
    const gardenGable=nz<-.9&&(wz<-131);
    const westGardenReturn=Math.abs(wx-247.7)<.1&&wz<-122.7;
    const count=gardenGable?1:westGardenReturn?2:Math.max(0,Math.floor((length-.8)/3.55));
    for(let n=0;n<count;n++){
      const t=gardenGable?.24:(n+.5)/count,x=a[0]+dx*t+nx*.035,z=a[1]+dz*t+nz*.035;
      if(nz<-.9&&Math.abs(z+cz+122.735)<.1&&bays.some(bay=>Math.abs(x+cx-bay.x)<3.35))continue;
      ...
      const covered=(Math.abs(x+cx-211.9)<.1&&z+cz<-122.6&&z+cz>-127.1)||
        (nz<-.9&&x+cx>236.1&&x+cx<247.8&&Math.abs(z+cz+122.735)<.1)||
        (Math.abs(x+cx-247.7)<.1&&z+cz>-132.1&&z+cz<-122.6);
  ...
  for(const [wx,wz,height] of chimneys){
  ...
  const conservatory={x0:236.5,x1:247.7,z0:-132,z1:-122.7};
  ...
  for(const [a,b] of [[[x0,z0],[x1,z0]],[[x0,z1],[x0,z0]]]){
    const dx=b[0]-a[0],dz=b[1]-a[1],len=Math.hypot(dx,dz),r=Math.atan2(-dz,dx),mid=[(a[0]+b[0])/2,(a[1]+b[1])/2];
    const wall=mesh(worldUV(new THREE.BoxGeometry(len,1.05,.2),1.7),brick,mid[0],.525,mid[1],'Conservatory low brick wall');wall.rotation.y=r;wall.userData.orientedCollision=true;
  ...
  building.userData={...building.userData,source:IRBY_ASHLEY,footprint:IRBY_ASHLEY_FOOTPRINT,storeys:2,openings,bays,conservatory,roofs:IRBY_ASHLEY_ROOFS,
