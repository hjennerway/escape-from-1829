...
for(const o of building.userData.openings){
  ...
  if(o.y<3.8&&o.x+building.position.x-offset.x>236&&o.x+building.position.x-offset.x<248&&o.z+building.position.z-offset.z<-122)continue;
...
assert(conservatory.z0-offset.z<-137.5-3,'Conservatory must project visibly past the west wing');
...
assert(pointInFootprint(placed([246,-125]),corner.footprint),'Quarter-octagonal corner must fill the old internal corner');
assert(!pointInFootprint(placed([244.2,-126]),corner.footprint),'Corner must retain its diagonal octagonal face');
...
assert(down(adjacentBay.x-offset.x,-124)?.object.name.includes('bay slate roof'),'Separate bay must retain its slate roof');
const cornerLeft=Math.min(...corner.footprint.map(p=>p[0]));
const intervening=openings.filter(o=>o.y>5&&Math.abs(o.z+building.position.z-offset.z+122.735)<.01&&o.x+building.position.x>bayRight&&o.x+building.position.x<cornerLeft);
assert.equal(intervening.length,2,'Exactly two exposed upper sashes must separate the half bay and quarter corner');
const wingPair=openings.filter(o=>o.y>5&&Math.abs(o.x+building.position.x-offset.x-247.665)<.01&&o.z+building.position.z-offset.z<-126.3);
...
for(const p of [[246,-125],[242,-142],[247.7,-140]])assert(obstacles.some(o=>obstacleContains(o,...placed(p))),'New corner and exposed conservatory walls must block walking: '+p);
assert(!obstacles.some(o=>obstacleContains(o,...placed([235.8,-128]))),'Access alongside the separate bay and conservatory must remain open');
for(const p of [[255,-104],[239,-104],[216,-107],[230,-117],[256,-133],[216,-129]])assert(obstacles.some(o=>obstacleContains(o,...placed(p))),'Walking must respect every wing');
...
for(const historic of [false,true])for(const modern of [false,true]){
  ...
  assert.equal(exteriorObstacles(THREE,exterior.model).some(o=>obstacleContains(o,...placed([255,-133]))),historic,'Hidden Historic building must not leave walking collisions');
}
