...
for(const p of [[255,-104],[239,-104],[216,-107],[216,-96],[230,-110],[256,-126],[216,-122]])assert(pointInFootprint(p,IRBY_ASHLEY_FOOTPRINT),'Refined wing and shifted rear range must be solid: '+p);
for(const p of [[245,-103],[228,-104],[232,-125],[266,-114],[206,-108],[208,-105]])assert(!pointInFootprint(p,IRBY_ASHLEY_FOOTPRINT),'Marked recess and removed yellow section must remain open: '+p);
const bounds=new THREE.Box3().setFromObject(building);
assert(bounds.min.x>203+offset.x&&bounds.max.x<264+offset.x&&bounds.min.z>-136+offset.z&&bounds.min.z<-134+offset.z&&Math.abs(bounds.max.z-IRBY_CONNECTION_FRONT)<.44,'Rear edge moves seven units while the front corridor contact stays fixed');
...
for(const p of [[255,-106],[239,-104],[215,-105],[216,-96],[231,-110],[256,-126],[216,-122]])assert(down(...p)?.point.y>=8.3,'Every main wing needs an upward-facing roof: '+p);
for(const p of [[245,-103],[228,-104],[232,-125]])assert(!down(...p),'Roof must not bridge a courtyard: '+p);
assert(down(242,-122)?.object.name.includes('Glazed'),'The photographed low glazed lean-to must occupy the left court');
building.traverse(o=>{
  if(!o.isMesh)return;
...
for(const o of building.userData.openings){
  ...
  if(o.y<3.8&&o.x+building.position.x-offset.x>236&&o.x+building.position.x-offset.x<248&&o.z+building.position.z-offset.z<-115)continue;
...
assert(conservatory.z0-offset.z<-130.5-3,'Conservatory must project visibly past the west wing');
assert(down(242,-133)?.object.name.includes('Glazed'),'Extended front must have a glazed roof');
assert(down(246,-118)?.object.name.includes('Quarter-octagonal'),'Corner must have its own hip roof');
// The marked corner peak must meet the west ridge without the former dip.
for(const t of [.25,.5,.75]){
  const hit=down(255.45-8.2*t,-117.02+.87*t);
...
assert(pointInFootprint(placed([246,-118]),corner.footprint),'Quarter-octagonal corner must fill the old internal corner');
assert(!pointInFootprint(placed([244.2,-119]),corner.footprint),'Corner must retain its diagonal octagonal face');
...
assert(down(adjacentBay.x-offset.x,-117)?.object.name.includes('bay slate roof'),'Separate bay must retain its slate roof');
const cornerLeft=Math.min(...corner.footprint.map(p=>p[0]));
const intervening=openings.filter(o=>o.y>5&&Math.abs(o.z+building.position.z-offset.z+115.735)<.01&&o.x+building.position.x>bayRight&&o.x+building.position.x<cornerLeft);
assert.equal(intervening.length,2,'Exactly two exposed upper sashes must separate the half bay and quarter corner');
const wingPair=openings.filter(o=>o.y>5&&Math.abs(o.x+building.position.x-offset.x-247.665)<.01&&o.z+building.position.z-offset.z<-119.3);
...
const near=new THREE.Vector3(242,15,-133),far=new THREE.Vector3(247,15,-133);
...
for(const p of [[246,-118],[242,-135],[247.7,-133]])assert(obstacles.some(o=>obstacleContains(o,...placed(p))),'New corner and exposed conservatory walls must block walking: '+p);
assert(!obstacles.some(o=>obstacleContains(o,...placed([235.8,-121]))),'Access alongside the separate bay and conservatory must remain open');
for(const p of [[255,-104],[239,-104],[216,-107],[230,-110],[256,-126],[216,-122]])assert(obstacles.some(o=>obstacleContains(o,...placed(p))),'Walking must respect every wing');
for(const p of [[255,-135],[216,-130],[230,-120],[242,-142]]){
...
for(const p of [...[[245,-103],[228,-104],[232,-132]].map(placed),IRBY_ASHLEY_VIEWS['irby-ashley-1'].position.filter((_,i)=>i!==1),IRBY_ASHLEY_VIEWS['irby-ashley-3'].position.filter((_,i)=>i!==1)])assert(!obstacles.some(o=>obstacleContains(o,...p)),'Courts and camera starts must remain accessible: '+p);
...
for(const historic of [false,true])for(const modern of [false,true]){
  ...
  assert.equal(exteriorObstacles(THREE,exterior.model).some(o=>obstacleContains(o,...placed([255,-126]))),historic,'Hidden Historic building must not leave walking collisions');
}
