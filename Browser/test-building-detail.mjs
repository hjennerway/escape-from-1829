...
assert.deepEqual([...hit.object.material.emissiveMap.image.data.slice(index,index+3)],[255,255,255],'Only the glass contributes to the distant glow');
const framePoint=new THREE.Vector3(-2,2.2,.3).applyMatrix4(facade.matrixWorld);
ray.set(framePoint.clone().addScaledVector(normal,2),normal.clone().negate());
const frameHit=ray.intersectObjects(visibleMeshes(testScene)).find(h=>h.object.userData.buildingWindowProxy);
const frameIndex=(Math.floor(frameHit.uv.y*width)*width+Math.floor(frameHit.uv.x*width))*4;
assert.equal(frameHit.object.material.emissiveMap.image.data[frameIndex],0,'Sash bars stay unlit');
assert.deepEqual([...new Set(hit.object.geometry.attributes.nightWindowId.array)],[1,2],'Proxy panes keep separate source identities');
