...
const trees=exterior.trees.children.filter(o=>o.userData.adminPineTree||o.userData.frontLawnTree||o.userData.oakTree);
assert.equal(trees.length,28);
for(const key of ['adminPineTree','frontLawnTree','oakTree']){
...
// The eleven added KML oaks each allow 11k triangles at distant detail.
assert(treeTriangles<180000+11*11000,'Distant trees should submit substantially less geometry');
