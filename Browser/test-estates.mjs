...
import {ESTATES_SOURCE_FOOTPRINT,ESTATES_FOOTPRINT,ESTATES_VIEWS,ESTATES_MAP_REFERENCE} from './dist/estates-department.mjs';
...
assert.deepEqual(building.position.toArray(),[248.9,0,-37],'Align the blue-circled Estates position to the fixed church and Churton');
const registered=wardMapPoint(ESTATES_MAP_REFERENCE.pixel);
assert(Math.hypot(registered[0]-building.position.x,registered[1]-building.position.z)<.08,'Estates matches the map centre to rounding precision');
