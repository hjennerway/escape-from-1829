...
const additions=KML_TREES.filter(t=>KML_12_ADDITIONS.some(p=>p.name===t.name&&p.coordinates[0]===t.longitude));
...
try{
 ...
 for(const mode of ['source','compiled']){
  ...
  for(const {year} of PERIODS){
   ...
   assert.equal(count,10,mode+' '+year+' shows all new trees');
  }
  for(const year of [1829,2021]){
   ...
   await page.screenshot({type:'jpeg',quality:80,path:fileURLToPath(new URL('kml-12-'+mode+'-'+year+'.jpg',import.meta.url))});
  }
 }
 assert.deepEqual(errors,[]);console.log('PASS: all ten new trees in every source/compiled period; 1829 and 2021 previews; no page errors.');
