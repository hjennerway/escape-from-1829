...
try{
  ...
  for(const [name,position,target,fov] of [
    ['east',[24,27,39],[28,10.8,18],36],
    ['west',[-24,27,39],[-28,10.8,18],36],
    ['east-rear',[11,33,-4],[24,12,18],36],
    ['east-close',[26,19.5,28],[28.8,11.4,18.5],44]
  ]){
    ...
    await page.screenshot({path:fileURLToPath(new URL(`roof-junctions-${stage}-${name}.jpg`,import.meta.url)),quality:90});
  }
  assert.deepEqual(errors,[]);
  console.log(`PASS: ${stage} both mirrored roof junctions and close corner views rendered from ${mode}.`);
