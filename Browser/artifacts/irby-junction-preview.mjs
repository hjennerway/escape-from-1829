...
const fit=JSON.parse(readFileSync(new URL('irby-junction-fit.json',import.meta.url)));
...
try{
 const page=await browser.newPage({viewport:{width:811,height:661}});page.setDefaultNavigationTimeout(120000);
 ...
 for(const name of ['marked','clear','plan']){
  await page.evaluate(({parameters,name})=>{
   ...
   if(name==='plan'){
    ...
    camera.fov=2*Math.atan(661/(2*f))*180/Math.PI;camera.updateProjectionMatrix();
    camera.projectionMatrix.elements[8]=1-2*cx/811;camera.projectionMatrix.elements[9]=2*cy/661-1;
  ...
  await page.screenshot({type:'jpeg',quality:70,path:new URL(`irby-junction-${tag}-${mode}-${name}.jpg`,import.meta.url).pathname.slice(1)});
 }
