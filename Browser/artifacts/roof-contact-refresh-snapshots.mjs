...
const oldArgs=['--import','./Browser/artifacts/roof-contact-before-loader.mjs'];
...
for(const [mode,loader] of [['normal',null],['entrance','larkton-original-entrance-loader.mjs'],['east','rear-side-original-east-loader.mjs']]){
 ...
 for(let i=0;i<before.length;i++){
  ...
  let value=saved;for(const k of old.key)value=value[k];assert.deepEqual(value,old.value,'Validate pre-repair baseline: '+path);
...
const path='Research/leighton-newton/protected-before.json',saved=JSON.parse(readFileSync(path));assert.deepEqual(saved.geometry,old);saved.geometry=now;writeFileSync(path,JSON.stringify(saved,null,2)+'\n');
execFileSync(process.execPath,[...oldArgs,'Browser/artifacts/snapshot-annexe-front.mjs'],{windowsHide:true});
execFileSync(process.execPath,['Browser/artifacts/snapshot-annexe-front.mjs','--save'],{windowsHide:true});
writeFileSync('Browser/artifacts/roof-contact-snapshot-refresh.json',JSON.stringify(changes,null,2)+'\n');console.log(changes);
