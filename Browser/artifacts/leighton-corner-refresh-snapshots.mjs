// Update only snapshots that matched the pre-change corner model. Never accept
...
const beforeArgs=['--import','./Browser/artifacts/leighton-corner-before-loader.mjs'];
const scope=args=>{const {openings,...rest}=run([...args,'Browser/artifacts/windows-scope.mjs']);return rest;};
assert.deepEqual(scope([]),scope(beforeArgs),'All non-ward geometry and original L ranges must match before any fixture refresh');
const report=[];
...
writeFileSync('Browser/artifacts/leighton-corner-snapshot-refresh.json',JSON.stringify(report,null,2)+'\n');console.log(JSON.stringify(report,null,2));
