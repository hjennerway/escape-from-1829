...
const fit=JSON.parse(readFileSync(new URL('fork-angle-fit.json',import.meta.url)));
...
try{
 const page=await browser.newPage({viewport:{width:781,height:649}});page.setDefaultNavigationTimeout(120000);
 ...
 for(const name of ['marked','clear','plan']){
  await page.evaluate(({parameters,name})=>{
   ...
   if(name==='plan'){
    ...
    camera.fov=2*Math.atan(649/(2*f))*180/Math.PI;camera.updateProjectionMatrix();
    camera.projectionMatrix.elements[8]=1-2*cx/781;camera.projectionMatrix.elements[9]=2*cy/649-1;
  ...
  await page.screenshot({type:'jpeg',quality:90,path:new URL(`fork-angle-${tag}-${mode}-${name}.jpg`,import.meta.url).pathname.slice(1)});
 }
