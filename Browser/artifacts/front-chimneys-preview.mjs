...
try{
  ...
  await page.route('**/aerial.html*',async route=>{
    ...
    await route.fulfill({response,body:(await response.text()).replace('updateRoadLabels(THREE,layouts.roads,exterior.camera,innerWidth,innerHeight);','').replace('function frame(){','window.__chimneys={exterior,renderer,controls};function frame(){')});
  });
  await page.goto(base+'/aerial.html?view=front&period=1829&models='+mode);
  await page.waitForFunction(()=>window.__chimneys?.renderer.info.render.frame>3);
  assert.equal(await page.evaluate(()=>window.__chimneys.exterior.modelBuild.mode),mode==='source'?'procedural':'compiled');
  await page.evaluate(()=>{
    const {exterior}=window.__chimneys;
  ...
  for(const [name,position,target] of [
    ['oblique',[24,36,46],[0,13,14]],
    ['front',[0,19,61],[0,13,14]],
    ['photo',[-29,10,52],[0,13,14]]
  ]){
    await page.evaluate(({position,target})=>{
      const {exterior,controls}=window.__chimneys;
    ...
    await page.screenshot({path:new URL('front-chimneys-'+stage+'-'+name+'.jpg',import.meta.url).pathname.replace(/^\/([A-Z]:)/,'$1')});
  }
  assert.deepEqual(errors,[]);
  console.log('PASS: '+stage+' entrance chimneys rendered from '+mode+' without browser errors.');
