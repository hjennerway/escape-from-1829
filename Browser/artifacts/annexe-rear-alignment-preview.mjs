...
try{
 ...
 const page=await browser.newPage({viewport:{width:1099,height:841}}),errors=[];
 ...
 for(const mode of modes){
  ...
  for(const [name,p,t,fov] of [
   ['alignment',[-1.631612,107.569876,-67.68697],[-2.071306,0,-51.17476],48]
  ]){
   ...
   await page.screenshot({path:fileURLToPath(new URL('annexe-rear-alignment-'+phase+'-'+mode+'-'+name+'.png',import.meta.url))});
  }
 }
