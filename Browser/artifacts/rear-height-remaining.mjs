...
for(const command of commands.slice(start)){
 ...
 writeFileSync(new URL('./rear-height-'+file+'.log',import.meta.url),(r.stdout??'')+(r.stderr??''));
 results.push({file,status:r.status});console.log((r.status===0?'PASS: ':'FAIL: ')+file);
}
writeFileSync(new URL('./rear-height-remaining.json',import.meta.url),JSON.stringify(results,null,2)+'\n');
