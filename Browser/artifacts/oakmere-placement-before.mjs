...
export function addOakmereCourt(THREE,{model,scale,brick,roof,material,worldUV,hipRoof}){
 ...
 // Red square: a single-storey square annex behind the service head, towards
 // its right end. Convert the head centre into the main wing's local frame.
 const hx=c*(-3)-s*(-12),hz=s*(-3)+c*(-12),headExtension=14,headEnd=11+headExtension,ax=hx+headEnd+3.45,az=hz-3.4;
 // The later blue circle selects the cross-head's court face, not the long
 // green wing. Lengthen its outward end and carry the low annex with it.
 ...
 wall(hx+14,hz+5.62,6.4,8.4,1.3,'Oakmere blue-face projecting bay brick walls');
 cap(hx+14,hz+3.5,7.4,5.8,8.4,2.5,'Oakmere blue-face projecting bay slate roof');
 for(const u of [12.4,15.5])for(const y of [2.35,6.4])window(hx+u,y,hz+6.28,1.45,2.7,0);
 for(const y of [.25,4.35,8.18])box(trim,hx+14,y,hz+6.3,6.5*scale,.3,.17,'Oakmere blue-face bay brick band');
 ...
 window(ax+1.6,1.75,az+3.51,1.5,3.4,0);
 ...
 for(const [x,z,h,r] of [[8.8,-5.2,4.8,1],[8.8,2.3,4.3,.95],[8.1,-8.1,5.4,1.1],[ax+3.9,az+3.4,5.1,.8],[ax-.1,az+3.9,4.6,.85],[hx+12,hz+7.2,4.5,.9],[hx+17.3,hz+7.1,5.2,1],[hx+22.8,hz+6.5,4.4,.95]]){
