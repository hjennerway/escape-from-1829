...
try{
 ...
 for(const [name,p,t,fov] of [
  ['plan',[0,285,20.01],[0,0,20],52],
  ['front',[0,36,150],[0,4,20],52],
  ['site',[0,470,110],[0,0,110],58]
 ]){
  await page.evaluate(async({p,t,fov})=>{
   const {exterior,controls}=window.__centre,{annexeSitePoint}=await import('/annexe.mjs');
   exterior.scene.fog.density=0;exterior.scene.traverse(o=>{if(o.isSprite)o.material.visible=false;});
   exterior.camera.position.set(...annexeSitePoint(...p));
   const target=annexeSitePoint(...t);exterior.camera.lookAt(...target);exterior.camera.fov=fov;exterior.camera.updateProjectionMatrix();controls.sync(target);
  },{p,t,fov});
  await page.evaluate(()=>new Promise(resolve=>requestAnimationFrame(()=>requestAnimationFrame(resolve))));
  await page.screenshot({path:fileURLToPath(new URL('annexe-centre-'+phase+'-'+mode+'-'+name+'.png',import.meta.url))});
 }
